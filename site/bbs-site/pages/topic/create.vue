...
export default {
  ...
  async asyncData({ $axios, query, store }) {
    ...
    if (nodeId) {
      try {
        currentNode = await $axios.get('/api/topic/node/' + nodeId)
      } catch (e) {
        console.error(e)
      }
    }
  ...
  methods: {
    async submitCreate() {
      ...
      try {
        ...
        // this.$message.error(e.response.data.error.message || e.message)
      ...
      if (this.config.topicCaptcha) {
        try {
          const ret = await this.$axios.get(
            '/api/captcha/request?captchaId=' + this.captchaId
          )
          this.captchaId = ret.captchaId
          this.captchaUrl =
            'data:' + ret.imgType + ';base64,' + ret.captchaCodeImg
        } catch (e) {
