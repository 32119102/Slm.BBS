...
export default {
  async asyncData({ $axios, params, error }) {
    ...
    const [userLiked, userFavorited, commentsPage, likeUsers] =
      await Promise.all([
        $axios.get('/api/user-like/liked/' + params.id + '?entityType=topic'),
        $axios.get(
          '/api/user-favorite/favorited/' + params.id + '?entityType=topic'
        ),
        $axios.get('/api/comment/comments/' + params.id + '?entityType=topic'),
        $axios.get('/api/topic/recent-liked/' + params.id),
      ])
    ...
    return {
      ...
      script: [
        ...
        },
        {
          ...
          callback: () => {
            this.htmlContent = LuteHelper.MarkdownStr(this.topic.content)
          },
        },
      ],
    }
  ...
  mounted() {
    const that = this
    LuteHelper.MarkdownStr(this.topic.content, function (htmlContent) {
      ...
      try {
        if (this.favorited) {
          await this.$axios.delete(
            '/api/user-favorite/d/' + topicId + '?entityType=topic'
          )
          this.favorited = false
          this.$message.success('已取消收藏')
        } else {
          await this.$axios.post('/api/topic/favorite/' + topicId)
          this.favorited = true
