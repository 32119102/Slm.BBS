...
export default {
  ...
  async asyncData({ $axios }) {
    const user = await $axios.get('/api/user/site-current')
    const form = { ...user }
    return {
      user,
      form,
  ...
  methods: {
    async setUsername() {
      try {
        ...
        this.$showErrorMsg(err.response.data.error)
      }
    },
    async setEmail() {
      try {
    ...
    async setPassword() {
      try {
        ...
        await this.$axios.post('/api/user/set-password', {
          password: new MD5().update(me.form.password).digest('hex'),
          rePassword: new MD5().update(me.form.rePassword).digest('hex'),
        })
        await this.reload()
    ...
    async updatePassword() {
      try {
        ...
        await this.$axios.post('/api/user/update-password', {
          oldPassword: new MD5().update(me.form.oldPassword).digest('hex'),
          password: new MD5().update(me.form.password).digest('hex'),
          rePassword: new MD5().update(me.form.rePassword).digest('hex'),
        })
        await this.reload()
    ...
    async reload() {
      this.user = await this.$axios.get('/api/user/site-current')
      this.form = { ...this.user }
    },
  },
}
