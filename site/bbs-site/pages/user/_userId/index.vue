            ...
            <div v-if="activeTab === 'topics'">
              ...
              >
                <load-more
                  ...
                  :url="'/api/topic/user-topics/' + user.id"
                >
            ...
            <div v-if="activeTab === 'articles'">
              ...
              >
                <load-more
                  ...
                  :url="'/api/article/user-articles/' + user.id"
                >
...
export default {
  async asyncData({ $axios, params, query, error }) {
    let user
    try {
      user = await $axios.get('/api/user/user-profile/' + params.userId)
    } catch (err) {
      error({
        statusCode: 404,
    ...
    if (activeTab === 'topics') {
      topicsPage = await $axios.get('/api/topic/user-topics/' + params.userId)
    } else if (activeTab === 'articles') {
      articlesPage = await $axios.get(
        '/api/article/user-articles/' + params.userId
      )
    }
    return {
      activeTab,
