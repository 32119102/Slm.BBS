            ...
            <div class="article-header">
              ...
              <div class="article-meta">
                ...
                >
                  ...
                  >
                    <nuxt-link :to="'/articles/tag/' + tag.tagId" class>{{
                      tag.tagName
                    }}</nuxt-link>
                  </span>
                </span>
      ...
      <div class="right-container">
        ...
        >
          ...
          <div class="widget-content article-related">
            <ul>
              <li v-for="a in relatedArticles" :key="'r-' + a.articleId">
                <nuxt-link
        ...
        <div v-if="nearlyArticles && nearlyArticles.length" class="widget">
          ...
          <div class="widget-content article-related">
            <ul>
              <li v-for="a in nearlyArticles" :key="'n-' + a.articleId">
                <nuxt-link
                  :to="'/article/' + a.articleId"
                  :title="a.title"
                  class="article-related-title"
...
import LuteHelper from '~/common/LuteHelper'
...
export default {
  async asyncData({ $axios, params, error }) {
    ...
    const [commentsPage, userFavorited, nearlyArticles, relatedArticles] =
      await Promise.all([
        $axios.get(
          '/api/comment/comments/' + article.articleId + '?entityType=article'
        ),
        $axios.get(
          '/api/user-favorite/favorited/' + params.id + '?entityType=article'
        ),
  ...
  head() {
    return {
      ...
      script: [
        ...
        {
          type: 'text/javascript',
          src: LuteHelper.luteScript,
          callback: () => {
            this.htmlContent = LuteHelper.MarkdownStr(this.article.content)
          },
        },
      ],
    }
  ...
  mounted() {
    const that = this
    LuteHelper.MarkdownStr(this.article.content, function (htmlContent) {
      ...
      try {
        if (this.favorited) {
          await this.$axios.delete(
            '/api/user-favorite/d/' + articleId + '?entityType=article'
          )
