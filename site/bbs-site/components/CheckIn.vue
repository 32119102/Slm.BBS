...
export default {
  ...
  methods: {
    async getCheckIn() {
      try {
        this.checkIn = await this.$axios.get(
          '/api/check-in/check-in/' +
            (this.user == null ? 0 : this.user.id || 0)
        )
      } catch (e) {
        console.log(e)
      }
    ...
    async doCheckIn() {
      ...
      try {
        await this.$axios.post('/api/check-in/check-in')
    ...
    async loadCheckInRank() {
      try {
        this.checkInRank = await this.$axios.get('/api/check-in/rank')
      } catch (e) {
        console.error(e)
      }
    },
