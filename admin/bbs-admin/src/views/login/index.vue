...
export default {
  ...
  methods: {
    async showCaptcha() {
      try {
        var ret = await this.axios.get(
          "/api/app/captcha/request?captchaId=" + this.loginForm.captchaId
        );
        this.loginForm.captchaId = ret.captchaId;
        this.loginForm.captchaUrl = "data:" + ret.imgType + ";base64," + ret.captchaCodeImg;
      } catch (e) {
        console.log(e);
    ...
    handleLogin() {
      this.$refs.loginForm.validate((valid) => {
        if (valid) {
          this.loading = true;
          var pwd = getmd5(this.loginForm.password);
          this.$store
            .dispatch("user/login", {
