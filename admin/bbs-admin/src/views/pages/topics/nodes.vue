<template>
  <section class="page-container">
    ...
    >
      ...
      <el-table-column prop="logo" label="图标" width="200">
...
export default {
  ...
  methods: {
    list() {
      ...
      this.axios
        .post("/api/app/topic-node/list", params)
        .then((data) => {
    ...
    addSubmit() {
      const me = this;
      this.axios
        .post("/api/app/topic-node", this.addForm)
        .then((data) => {
    ...
    editSubmit() {
      const me = this;
      this.axios
        .put("/api/app/topic-node/" + me.editForm.id, me.editForm)
        .then((data) => {
          me.list();
    ...
    handleDel(index, row) {
      ...
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
        ...
        .then(() => {
          this.axios
            .delete("/api/app/topic-node/" + row.id)
            .then((data) => {
