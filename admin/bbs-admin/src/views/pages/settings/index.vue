...
export default {
  ...
  methods: {
    async load() {
      this.loading = true;
      try {
        this.config = await this.axios.get("/api/app/sys-config");
        // this.nodes = await this.axios.get("/api/admin/topic-node/nodes");
        var _nodes = await this.axios.post("/api/app/topic-node/list", {
          skipCount: 0,
          maxResultCount: 100,
        });
        this.nodes = _nodes.items;
      } catch (err) {
    ...
    async save() {
      ...
      try {
        ...
        await this.axios.post("/api/app/sys-config/save", this.config);
